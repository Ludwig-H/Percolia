"""Vendor the actual defense TikZ, changing only labels and poster typography.
Source coordinates, combinatorics and tree topology are retained. Ordinary
XeLaTeX builds use the committed figures and do not need the network.
"""
...
def adapt(name: str, text: str) -> str:
    ...
    if name == 'comp_dendro_rsl.tex':
        text = text.replace(r'Seuil $\epsilon$', r'$\epsilon$')
        # Balls have radius r; all short edges have length 2r.
        text = text.replace('{$r$}', '{$2r$}')
    elif name == 'comp_dendro_2nn.tex':
        text = text.replace(r'Seuil $\epsilon$', r'$\rho$')
...
def main() -> None:
    ...
    parser.add_argument('--force', action='store_true', help='Explicitly regenerate already-vendored figures')
