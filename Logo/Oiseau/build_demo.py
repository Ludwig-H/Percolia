...
def inline_svg(path: Path, width: int, height: int, extra_class: str = "") -> str:
    ...
    text = text.replace('<svg ', f'<svg width="{width}" height="{height}" overflow="visible" class="{extra_class}" ' , 1)
    return text
...
def main() -> None:
    ...
    compact = inline_svg(ROOT / "percolia-bird-compact.svg", 520, 292, "network-bird-svg network-bird-svg--compact")
    word_style, wordmark = extract_group(LOGO_ROOT / "Police" / "percolia-wordmark-primary.svg", "percolia-wordmark")
...
<title>Percolia — vol directionnel de l’oiseau-réseau</title>
<style>
:root {{
  --ink:#082C4C;
  --blue:#1C83D4;
...
.lead {{ margin:10px 0 0; color:#355874; max-width:76ch; font-size:clamp(1rem,1.5vw,1.22rem); line-height:1.58; }}
...
.status {{ min-width:90px; text-align:right; color:var(--slate); font-size:.8rem; letter-spacing:.12em; text-transform:uppercase; }}
...
<header>
  <div>
    <p class="kicker">Percolia · direction 03</p>
    <h1>L’oiseau-réseau retrouve sa forme initiale</h1>
    <p class="lead">Un premier oiseau quitte le <strong>P</strong> et traverse la scène dans une seule direction. Il disparaît hors champ. Après un court silence, un autre arrive depuis la direction opposée et revient se percher sur le <strong>P</strong>.</p>
  </div>
...
<svg id="flight-stage" class="percolia-flight-stage" data-flight-stage="true" viewBox="0 0 1360 760" role="img" aria-labelledby="scene-title scene-desc">
  ...
  <desc id="scene-desc">Un oiseau part vers la droite et quitte la scène. Un autre arrive de droite vers la gauche et se pose sur le P.</desc>
  <defs>
  ...
  <g opacity=".54">
    <circle cx="124" cy="154" r="2.4" fill="#1C83D4"/>
    <circle cx="205" cy="105" r="3" fill="#20C9C4"/>
    <circle cx="515" cy="116" r="2" fill="#082C4C"/>
    <circle cx="747" cy="95" r="2.5" fill="#20C9C4"/>
    <circle cx="1050" cy="132" r="2.3" fill="#1C83D4"/>
    <circle cx="1190" cy="265" r="3" fill="#20C9C4"/>
  </g>

  <g id="wordmark" transform="translate(140 500) scale(1)">
  ...
  <g id="perched-bird" data-perched-bird="true" transform="translate({perch_x} {perch_y}) scale({perched_scale}) translate({-local_perch[0]} {-local_perch[1]})">
    {compact}
  </g>

  <g id="outbound-bird" data-flight-bird="outbound" opacity="0">
    {primary}
  </g>
  <g id="inbound-bird" data-flight-bird="inbound" opacity="0">
    {primary}
  </g>
...
<section class="info" aria-label="Principes de l’animation">
  <article class="card"><h2>Silhouette restaurée</h2><p>Le corps, la tête et la queue reprennent la topologie triangulée du premier jet, au lieu du modèle paramétrique lisse qui avait perdu toute sa personnalité.</p></article>
  <article class="card"><h2>Ailes déformables</h2><p>Épaule, coude, poignet et extrémité définissent une chaîne à trois segments. L’aile se déploie à la descente et se replie pendant la remontée.</p></article>
  <article class="card"><h2>Deux oiseaux, deux directions</h2><p>L’oiseau sortant ne fait jamais demi-tour. Un second objet SVG apparaît depuis le bord opposé et réalise l’approche puis l’atterrissage.</p></article>
</section>
...
(() => {{
  ...
  const controller = initPercoliaDirectionalScene({{
    ...
    autoplay: true,
    onState: (state) => {{
      const labels = {{perched:'PERCHÉ',takeoff:'DÉCOLLAGE',outbound:'SORTIE',empty:'HORS CHAMP',inbound:'RETOUR',landing:'ATTERRISSAGE'}};
