...
Les contrôles portent sur l’A0, les polices incorporées, l’ordre des formules, l’absence de ρ, la numérotation 1–7, le contenu à l’intérieur des blocs, les **espacements mesurés dans le PDF**, les deux séparateurs Perspective et l’absence de panneaux blancs, l’ordre des trois parties et la marge du texte autour des lignes, la taille de l’image Naval Group, le QR unique et l’exemple géométrique. Ils complètent l’inspection visuelle. Les auxiliaires restent dans `build/`, ignoré par Git. `make png` matricialise le PDF via `render_png.py` (PyMuPDF) ; dans l'enchaînement `make all check preview png` il vient après les contrôles, mais la cible elle-même ne dépend pas de `check`. Il refuse un PDF qui n'a pas exactement une page, un format autre que l'A0 portrait à 0,1 mm près, un rendu sous 55 dpi et une taille de raster inattendue ; il n'ajoute, ne retire ni ne recompose aucun contenu. Le rendu est déterministe à PDF constant — deux exécutions donnent le même sha256 — mais le PDF ne l'est pas : `make_figures.py` rejoue HDBSCAN à chaque passage et quelques pixels bougent, si bien que le workflow recommite un blob PNG de ~5,8 Mio à chaque exécution, en plus du PDF et de l'aperçu. Le workflow reconstruit et enregistre les livrables sur `main`.
